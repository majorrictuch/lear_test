...
namespace Lear.CRS.Extensions
{
    ...
    /// SqlSugar 启动服务
    /// </summary>
    public static class SqlsugarDevSetup
    {
        public static void AddSqlsugarDevSetup(this IServiceCollection services)
        {
            #region 配置sqlsuagr
            List<ConnectionConfig> connectConfigList = new List<ConnectionConfig>();
            //数据库序号从0开始,默认数据库为0
          //  _cfg.GetSection("Mysql").Get<MysqlConfig>()
            var config = Appsettings.app(new string[] { "ConnectionStrings", "DefaultDbString_Dev" });
